...
fun SponsorTrashScreen(
                ...
                trashedByYear.forEach { group ->
                    item(key = "header_${group.jahr}") {
                        Row(
                            ...
                            Text("${group.jahr} (${group.sponsors.size})", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
                            Row {
                                TextButton(onClick = { viewModel.restoreYear(group.jahr) }) { Text("Wiederherstellen") }
                    ...
                    items(group.sponsors, key = { it.id }) { sponsor ->
                        Card(modifier = Modifier.fillMaxWidth()) {
                            Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                                Text("${sponsor.wer} – ${sponsor.sorte}", style = MaterialTheme.typography.bodyMedium)
                                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                    TextButton(onClick = { viewModel.restoreSponsor(sponsor) }) { Text("Wiederherstellen") }
                                    TextButton(onClick = { confirmDeleteSponsor = sponsor }) {
                                        Text("Endgültig löschen", color = MaterialTheme.colorScheme.error)
                                    }
                                }
                            }
    ...
    confirmDeleteSponsor?.let { sponsor ->
        AlertDialog(
            ...
            text = { ScaledContent(factory) { Text("\"${sponsor.wer} – ${sponsor.sorte}\" wird unwiderruflich gelöscht.") } },
            confirmButton = {
                ScaledContent(factory) {
                    TextButton(onClick = { viewModel.deleteSponsorPermanently(sponsor); confirmDeleteSponsor = null }) {
    ...
    confirmDeleteYear?.let { jahr ->
        AlertDialog(
            ...
            text = { ScaledContent(factory) { Text("Alle Sponsoren aus $jahr werden unwiderruflich gelöscht.") } },
            confirmButton = {
                ScaledContent(factory) {
