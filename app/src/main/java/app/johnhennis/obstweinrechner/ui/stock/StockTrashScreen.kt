...
fun StockTrashScreen(
                ...
                trashedByYear.forEach { group ->
                    item(key = "header_${group.jahr}") {
                        Row(
                            ...
                            Text("${group.jahr} (${group.items.size})", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
                            Row {
                    ...
                    items(group.items, key = { it.id }) { item ->
                        Card(modifier = Modifier.fillMaxWidth()) {
                            Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                                Text("${item.art} (${item.quelle})", style = MaterialTheme.typography.bodyMedium)
                                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                    TextButton(onClick = { viewModel.restoreItem(item) }) { Text("Wiederherstellen") }
                                    TextButton(onClick = { confirmDeleteItem = item }) {
                                        Text("Endgültig löschen", color = MaterialTheme.colorScheme.error)
                                    }
                                }
                            }
    ...
    confirmDeleteItem?.let { item ->
        AlertDialog(
            ...
            text = { ScaledContent(factory) { Text("\"${item.art}\" wird unwiderruflich gelöscht.") } },
            confirmButton = {
                ScaledContent(factory) {
                    TextButton(onClick = { viewModel.deleteItemPermanently(item); confirmDeleteItem = null }) {
    ...
    confirmDeleteYear?.let { jahr ->
        AlertDialog(
            ...
            text = { ScaledContent(factory) { Text("Alle Positionen aus $jahr werden unwiderruflich gelöscht.") } },
            confirmButton = {
                ScaledContent(factory) {
